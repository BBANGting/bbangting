...
public class AuthService {
    ...
    // RT를 생성한 후 Redis에 {key:RT({발급자}):{email}, value:{RT}} 형식으로 저장
    ...
    public UserTokenDto login(LoginRequestDto requestDto) {
        ...
        return generateToken(SERVER, authenticationToken.getName(), getAuthorities(authentication));

    }

    // Header의 AT가 만료일자만 초과한 유효한 토큰인지 검사
    ...
    @Transactional
    public UserTokenDto reissue(String requestAccessTokenInHeader, String requestRefreshToken) {
        String requestAccessToken = resolveToken(requestAccessTokenInHeader);
        ...
        UserTokenDto tokenDto = jwtTokenProvider.createToken(principal, authorities);
    ...
    // 토큰 발급
    @Transactional
    public UserTokenDto generateToken(String provider, String email, String authorities) {
        // RT가 이미 있을 경우
        ...
        UserTokenDto tokenDto = jwtTokenProvider.createToken(email, authorities);

...
import com.khu.bbangting.jwt.repository.RefreshTokenRepository;
...
public class TokenProvider {
    ...
    public TokenDto generateTokenDto(UserDetailsImpl userDetails) {
        ...
        System.out.println("TokenProvier 진입");

        // access token 생성 (사용자 정보를 복호화)
        Date accessTokenExpiresIn = new Date(now + ACCESS_TOKEN_EXPIRE_TIME);
        String accessToken = Jwts.builder()
                .setSubject(userDetails.getUsername())                         // payload "sub": "email"
                .claim(AUTHORITIES_KEY, Role.USER.toString())               // payload "auth" : "USER"
    ...
    public UserDetailsImpl getUserFromAuthentication() {
    ...
    public String validateRefreshToken(HttpServletRequest request) {
        try {
            ...
            Date accessTokenExpiresIn = new Date(now + ACCESS_TOKEN_EXPIRE_TIME);
            String accessToken = Jwts.builder()
                    .setSubject(token.getUser().getEmail())                // payload "sub": "email"
                    .claim(AUTHORITIES_KEY, Role.USER.toString())   // payload "auth" : "USER
                    .setExpiration(accessTokenExpiresIn)                        // payload "exp" : 만료시간 설정
                    .signWith(key, SignatureAlgorithm.HS256)                    // header "alg" : "HS512"
                    .compact();
